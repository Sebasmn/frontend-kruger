...
export class CreateUserComponent implements OnInit {
  ...
  insertUser(): void {
    ...
    if(this.employeeForm.valid){
      ...
      let resValidatedCard: boolean = this.validateUserCard(cardU);
      if(resValidatedCard){
        // Abrimos nuestro componente de carga
        this.loadingState = true;
        //Actualizamos la informacion del usuario
        this.userService.registerNewEmployee(cardU, namesU, lastNamesU, emailU).subscribe(
          data => {
            this.openSuccessCreate();
          },
          err => {
            this.openCreateError(err.error.message);
            this.loadingState = false;
          }
        );
      }else{
        this.openCreateError("La cédula ingresada no es válida");
        this.loadingState = false;
      }

    }else{
      this.openCreateError("Debes ingresar todos los datos");
      this.loadingState = false;
    }
  }
  ...
  openCreateError(mensaje: string) : void {
    Swal.fire({
      icon: 'error',
